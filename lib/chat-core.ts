// 챗 API 공유 코어 — Cloudflare Pages Function(functions/api/chat.ts)과
// Vercel/Next 라우트 핸들러(app/api/chat/route.ts) 양쪽에서 재사용.
// Gemini 2.5 Flash 프록시. 표준 Web API(Request/Response/fetch)만 사용해 플랫폼 무관.

import { KNOWLEDGE_BASE, KNOWLEDGE_BASE_VERSION } from "./data/knowledge-base";
...
export const SYSTEM_PROMPT = `당신은 정인수님의 포트폴리오 사이트에 내장된 AI 어시스턴트입니다.
...
/**
 * 플랫폼 무관 챗 처리기. Cloudflare Function / Next Route Handler 양쪽에서 호출.
 * @param request 표준 Web Request (POST, JSON body: { messages })
 * @param apiKey  GEMINI_API_KEY (Cloudflare env 또는 process.env)
 */
